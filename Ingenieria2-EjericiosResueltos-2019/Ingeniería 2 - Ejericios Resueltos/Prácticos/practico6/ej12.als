...
assert lookup_doesnt_change {
	 init [first]
	 all b: Book - last | let b' = next [b] |
		 some n,m: Name, t: Target | ( n!=m and  some lookup[b, m] and (add [b, b', n, t] or some lookup[b', m]) ) 
																																=> ( lookup[b',m] = lookup[b,m])
	 }

check lookup_doesnt_change for 5 but 2 Book
